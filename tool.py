...
BASE_URL   = "https://vkhanhxhthuminhthu.onrender.com"
_DIR       = os.path.dirname(os.path.abspath(__file__))
TOOL_FILE  = os.path.join(_DIR, "tool_data.txt")   # 1 file duy nhat: hwid + key
...
# ════════════════════════════════════════════════════════════
#  QUAN LY 1 FILE DUY NHAT: tool_data.txt
#  Luu: hwid (co dinh), key, expiry, is_permanent
# ════════════════════════════════════════════════════════════
...
def _gen_hwid():
    """Tao HWID tu MAC address. Fallback = hostname+platform."""
    try:
        mac = uuid.getnode()
        # getnode that bai: tra 0 hoac random MAC (bit multicast set)
        if mac and mac != 0 and not (mac >> 40 & 0x01):
            return hashlib.sha256(str(mac).encode('utf-8')).hexdigest()[:20].upper()
    except Exception:
        pass
    import platform
    seed = platform.node() + platform.system() + platform.machine()
    return hashlib.sha256(seed.encode('utf-8')).hexdigest()[:20].upper()

def get_hwid():
    """HWID co dinh: doc tu file. Neu chua co thi tao moi va luu ngay."""
    data = _load_raw()
    hwid = data.get('hwid', '')
    if hwid and len(hwid) == 20 and hwid.isalnum():
        return hwid
    hwid = _gen_hwid()
    data['hwid'] = hwid
    _save_raw(data)
...
def save_auth(hwid, key, expiry, is_permanent):
    """Luu thong tin xac thuc vao file, giu nguyen hwid."""
    data = _load_raw()
    data['hwid']            = hwid
    data['key']             = key
    data['expiry']          = expiry       # Unix timestamp hoac -1 neu vinh vien
    data['is_permanent']    = is_permanent # True/False chinh xac tu server
    data['server_verified'] = True         # Danh dau da xac thuc thanh cong voi server
    _save_raw(data)

def clear_auth():
    """Xoa thong tin key/expiry nhung giu nguyen hwid."""
    data = _load_raw()
    hwid = data.get('hwid', '')
    _save_raw({'hwid': hwid})

def load_auth():
    """Tra ve (key, expiry, is_permanent) tu file."""
    data = _load_raw()
    return data.get('key',''), data.get('expiry', 0), data.get('is_permanent', False)


# ════════════════════════════════════════════════════════════
#  KIEM TRA HAN SU DUNG — DUNG LOGIC K.PY
#
#  Quy tac chinh xac:
#    is_permanent = True              → hop le vinh vien
#    is_permanent = False
#        expiry > 0 va time < expiry  → hop le co han
#        bat ky truong hop khac        → het han / chua co
#
#  KHONG bao gio coi expiry<=0 la "vinh vien" neu is_permanent=False
# ════════════════════════════════════════════════════════════

def is_auth_valid():
    """
    Kiem tra thong tin da luu co con hop le khong.
    Tra ve: (valid: bool, key: str, expiry: float, is_permanent: bool)
    """
    key, expiry, is_perm = load_auth()
    if not key:
        return False, key, expiry, is_perm
    # Truong hop vinh vien: chi khi server xac nhan is_permanent=True
    if is_perm:
        return True, key, expiry, is_perm
    # Truong hop co han: phai co expiry duong va chua het gio
    if expiry is not None and expiry > 0 and time.time() < expiry:
        return True, key, expiry, is_perm
    # Truong hop server khong tra ve expiry (expiry=0 hoac None):
    # Neu da tung duoc server xac nhan thanh cong (server_verified=True),
    # van cho phep vao tool — server se kiem tra lai khi khoi dong.
    # Chi can 1 trong 2: nhap Key HOAC duoc Admin duyet ID.
    if not expiry or expiry == 0:
        raw = _load_raw()
        if raw.get('server_verified', False):
            return True, key, expiry, is_perm
    # Tat ca truong hop con lai = het han hoac chua xac thuc
    return False, key, expiry, is_perm
...
def format_time_left(seconds):
    if seconds <= 0: return "Het han"
    d = int(seconds // 86400)
    h = int((seconds % 86400) // 3600)
    m = int((seconds % 3600) // 60)
    s = int(seconds % 60)
    parts = []
    if d: parts.append(f"{d} ngay")
    if h: parts.append(f"{h} gio")
    if m: parts.append(f"{m} phut")
    if s: parts.append(f"{s} giay")
    return ' '.join(parts) if parts else '< 1 giay'

def loading_spinner(stop_evt, text="Dang xu ly..."):
    chars = ['|', '/', '-', '\\']
    i = 0
    while not stop_evt.is_set():
        sys.stdout.write(f'\r{C_CYAN}[{chars[i]}] {text}{C_RESET}')
        sys.stdout.flush()
        time.sleep(0.1)
        i = (i + 1) % 4
...
def spin(text, fn, *args, **kwargs):
    """Chay fn voi loading spinner. Tra ve ket qua."""
    stop = threading.Event()
    res = [None]; err = [None]
    def w():
        try: res[0] = fn(*args, **kwargs)
        except Exception as e: err[0] = e
    t1 = threading.Thread(target=loading_spinner, args=(stop, text))
    t2 = threading.Thread(target=w)
    t1.start(); t2.start(); t2.join(); stop.set(); t1.join()
    if err[0]: raise err[0]
    return res[0]

def show_banner(hwid, key=None, expiry=0, is_perm=False):
    clear_screen()
    print(f"{C_PURPLE}{'='*54}{C_RESET}")
    print(f"{C_CYAN}{C_BOLD}{'  TOOL SPAM OTP VIP - VAN KHANH  ':^54}{C_RESET}")
    print(f"{C_PURPLE}{'='*54}{C_RESET}")
    print(f"  {C_YELLOW}HWID: {C_GREEN}{hwid}{C_RESET}")
    if key and key not in ("", "__da_duyet__"):
        print(f"  {C_CYAN}KEY : {C_YELLOW}{key}{C_RESET}")
    if key:
        # Chi hien "Vinh vien" khi is_permanent=True chinh xac tu server
        if is_perm:
            print(f"  {C_CYAN}HAN : {C_GREEN}Vinh vien{C_RESET}")
        elif expiry and expiry > 0:
            left = expiry - time.time()
            if left > 0:
                print(f"  {C_CYAN}CON : {C_YELLOW}{format_time_left(left)}{C_RESET}")
            else:
                print(f"  {C_CYAN}CON : {C_RED}HET HAN!{C_RESET}")
        # Neu khong co expiry ro rang: khong hien gi them
    print(f"{C_PURPLE}{'-'*54}{C_RESET}")

def notify_expired():
    clear_screen()
    print(f"{C_RED}{'='*54}{C_RESET}")
    print(f"{C_RED}  [!] KEY / DUYET DA HET HAN!{C_RESET}")
    print(f"{C_RED}  Lien he Admin de mua hoac gia han.{C_RESET}")
    print(f"{C_RED}{'='*54}{C_RESET}")
    clear_auth()
    time.sleep(2)
...
def api_verify(key, hwid):
    r = requests.post(f"{BASE_URL}/api/verify",
                      json={"key": key, "hwid": hwid}, timeout=15)
    return r.json()

def api_getkey(hwid):
    r = requests.get(f"{BASE_URL}/api/getkey", params={"hwid": hwid}, timeout=15)
    return r.json()

def api_submit_device(hwid):
    r = requests.post(f"{BASE_URL}/api/submit_device_request",
                      json={"device_id": hwid, "val": "1",
                            "unit": "ngay", "note": "Tool auto"},
                      timeout=15)
    return r.json()

def api_check_device(hwid):
    r = requests.post(f"{BASE_URL}/api/check-device",
...
# --- MENU 1: NHAP KEY ---
def menu_nhap_key(hwid):
    """Xac thuc bang Key. Tra ve True neu thanh cong."""
    show_banner(hwid)
    print(f"{C_YELLOW}  Nhap Key de kich hoat tool{C_RESET}\n")
    key = input(f"{C_YELLOW}[?] Nhap Key: {C_RESET}").strip()
    if not key:
        print(f"{C_RED}[!] Chua nhap Key!{C_RESET}")
        return False
    try:
        data = spin("Dang xac thuc Key voi Server...", api_verify, key, hwid)
    except Exception:
        print(f"\n{C_RED}[!] Loi ket noi! Thu lai sau.{C_RESET}")
        return False
    st = data.get('status')
    if st == 'success':
        # Lay chinh xac tu server — KHONG dung "or 0"
        raw_exp = data.get("expiry")         # co the la int/float/-1/None
        is_perm = bool(data.get("is_permanent", False))
        # Chuyen doi an toan
        try:
            expiry = float(raw_exp) if raw_exp is not None else 0.0
        except (TypeError, ValueError):
            expiry = 0.0
        save_auth(hwid, key, expiry, is_perm)
        show_banner(hwid, key, expiry, is_perm)
        print(f"{C_GREEN}[OK] Xac thuc THANH CONG!{C_RESET}")
        if is_perm:
            print(f"    Han: {C_GREEN}Vinh vien (VIP Toi Thuong){C_RESET}")
        else:
            print(f"    Het han  : {C_CYAN}{data.get('expiry_str','N/A')}{C_RESET}")
            print(f"    Con lai  : {C_YELLOW}{data.get('time_left','N/A')}{C_RESET}")
        if data.get('is_new_device'):
            print(f"\n{C_CYAN}[i] Thiet bi moi da duoc dang ky thanh cong!{C_RESET}")
        return True
    if st == 'expired':
        clear_auth()
        print(f"{C_RED}[X] Key da HET HAN!{C_RESET}")
        print(f"    Het han: {C_YELLOW}{data.get('expiry_str','N/A')}{C_RESET}")
        print(f"    Vui long lien he Admin mua Key moi.{C_RESET}")
    elif st == 'device_limit':
        print(f"{C_RED}[X] Da dat gioi han thiet bi toi da{C_RESET}")
        print(f"    Key nay chi ho tro {C_YELLOW}{data.get('max_devices','?')}{C_RED} thiet bi.{C_RESET}")
    elif st == 'invalid':
        print(f"{C_RED}[X] Key KHONG TON TAI!{C_RESET}")
    else:
        print(f"{C_RED}[!] Loi: {data.get('message','Khong xac dinh')}{C_RESET}")
    return False

# --- MENU 2: TAO LINK NHAN KEY ---
def menu_tao_link(hwid):
    show_banner(hwid)
    print(f"{C_YELLOW}  Tao link nhan Key mien phi (max 2 link/ngay){C_RESET}\n")
    try:
        data = spin("Dang tao link...", api_getkey, hwid)
    except Exception:
        print(f"{C_RED}[!] Loi ket noi!{C_RESET}")
        return
    st = data.get('status')
    if st == 'success':
        print(f"{C_GREEN}Link: {data.get('link','')}{C_RESET}")
    elif st == 'limit':
        print(f"{C_RED}[!] Da dat gioi han 2 link/ngay.{C_RESET}")
    else:
        print(f"{C_RED}[!] Loi: {data.get('message','')}{C_RESET}")

# --- MENU 3: GUI HWID CHO ADMIN ---
def menu_gui_id(hwid):
    show_banner(hwid)
    print(f"{C_YELLOW}  Gui HWID cho Admin duyet (khong can nhap Key){C_RESET}")
    print(f"  {C_CYAN}HWID: {C_GREEN}{hwid}{C_RESET}\n")
    try:
        data = spin("Dang gui yeu cau len Server...", api_submit_device, hwid)
    except Exception:
        print(f"{C_RED}[!] Loi ket noi!{C_RESET}")
        return
    st = data.get('status')
    if st == 'success':
        print(f"{C_GREEN}[OK] Da gui yeu cau thanh cong!{C_RESET}")
        print(f"     HWID  : {C_YELLOW}{hwid}{C_RESET}")
        print(f"     Ma YC : {C_CYAN}{data.get('req_id','N/A')}{C_RESET}")
        print(f"\n{C_YELLOW}Cho Admin duyet → bam [4] kiem tra → bam [5] vao tool.{C_RESET}")
    elif st == 'exists':
        print(f"{C_YELLOW}[i] HWID dang cho duyet. Bam [4] de kiem tra.{C_RESET}")
    elif st == 'already_approved':
        print(f"{C_GREEN}[OK] HWID da duoc duyet! Bam [4] xac nhan → [5] vao tool.{C_RESET}")
    else:
        print(f"{C_RED}[!] Loi: {data.get('msg', data.get('message',''))}{C_RESET}")

# --- MENU 4: KIEM TRA ADMIN DA DUYET (THEO K.PY) ---
def menu_check_duyet(hwid):
    """
    Kiem tra Admin da duyet HWID chua. Logic theo k.py:
    - approved : hien time_left, expiry_str tu server → luu → tra ve True
    - expired  : thong bao het han, xoa auth
    - not_found: chua duoc duyet
    """
    show_banner(hwid)
    print(f"{C_YELLOW}  Kiem tra Admin da duyet HWID chua{C_RESET}")
    print(f"  {C_CYAN}HWID: {C_GREEN}{hwid}{C_RESET}\n")
    try:
        data = spin("Dang tra cuu du lieu tren Server...", api_check_device, hwid)
    except Exception:
        print(f"{C_RED}[!] Loi ket noi!{C_RESET}")
        return False
    st = data.get('status')
    if st == 'approved':
        # Lay chinh xac tu server — KHONG tu y gan vinh vien
        ...
        try:
            ...
            expiry = 0.0
        kf = data.get('key') or '__da_duyet__'
        save_auth(hwid, kf, expiry, is_perm)
        show_banner(hwid, kf, expiry, is_perm)
        print(f"{C_GREEN}[OK] Thiet bi DA DUOC ADMIN DUYET!{C_RESET}")
        if kf != "__da_duyet__":
            print(f"    Kich hoat qua Key: {C_CYAN}{kf}{C_RESET}")
        print(f"    Thoi gian con lai: {C_YELLOW}{data.get('time_left','N/A')}{C_RESET}")
        print(f"    Het han vao luc  : {C_CYAN}{data.get('expiry_str','N/A')}{C_RESET}")
        print(f"\n{C_GREEN}[>>>] Ban co the su dung Tool binh thuong! Bam [5] vao tool.{C_RESET}")
        return True
    if st == 'expired':
        print(f"{C_RED}[X] Thiet bi cua ban DA HET HAN!{C_RESET}")
        print(f"    Het han vao luc: {C_YELLOW}{data.get('expiry_str','N/A')}{C_RESET}")
        print(f"    Vui long lien he Admin de gia han them.{C_RESET}")
        clear_auth()
    elif st == 'not_found':
        print(f"{C_RED}[!] Thiet bi chua duoc duyet hoac chua dang ky Key!{C_RESET}")
        print(f"    Chon [1] nhap Key, hoac [3] gui HWID cho Admin duyet.{C_RESET}")
    else:
        print(f"{C_RED}[!] Loi he thong: {data.get('message','Khong xac dinh')}{C_RESET}")
    return False


# ════════════════════════════════════════════════════════════
#  LUONG XAC THUC CHINH
# ════════════════════════════════════════════════════════════

def run_auth(hwid):
    """
    Xac thuc truoc khi vao tool.
    - Da co auth hop le (local + server) → vao luon
    - Het han → thong bao, quay lai menu
    - Chua co → hien menu auth
    Tra ve True khi cho phep vao tool.
    """
    valid, saved_key, saved_exp, saved_perm = is_auth_valid()
    if valid:
        # Xac thuc lai voi server (nhanh, tranh gia mao)
        try:
            if saved_key and saved_key != "__da_duyet__":
                # Dung key xac thuc
                data = spin("Dang kiem tra key...", api_verify, saved_key, hwid)
                st   = data.get('status')
                if st == 'success':
                    raw_exp = data.get("expiry")
                    is_p    = bool(data.get('is_permanent', False))
                    try:
                        exp = float(raw_exp) if raw_exp is not None else 0.0
                    except (TypeError, ValueError):
                        exp = 0.0
                    save_auth(hwid, saved_key, exp, is_p)
                    return True   # AUTO VAO TOOL
                elif st == 'expired':
                    clear_auth()
                    notify_expired()
                    # Tiep tuc xuong menu
                ...
                # Dung device approval
                data = spin("Dang xac thuc thiet bi...", api_check_device, hwid)
                st   = data.get('status')
                if st == 'approved':
                    raw_exp = data.get("expiry")
                    is_p    = bool(data.get('is_permanent', False))
            ...
            # Mat mang: tin tuong local (nhung local da valid)
            return True

    # ── MENU XAC THUC ──────────────────────────────────────
    verified = False
    while True:
        v2, k2, e2, p2 = is_auth_valid()
        unlocked = verified or v2
        show_banner(hwid, k2 if unlocked else None, e2, p2)
        print(f"  {C_GREEN}1.{C_RESET} Nhap Key VIP / Key Free")
        print(f"  {C_GREEN}2.{C_RESET} Tao link nhan Key mien phi")
        print(f"  {C_GREEN}3.{C_RESET} Gui HWID cho Admin duyet")
        print(f"  {C_GREEN}4.{C_RESET} Kiem tra Admin da duyet chua")
        if unlocked:
            print(f"  {C_GREEN}{C_BOLD}5. VAO TOOL{C_RESET}")
        print(f"  {C_RED}0.{C_RESET} Thoat")
        print(f"{C_PURPLE}{'-'*54}{C_RESET}")
        prompt = f"{C_YELLOW}Chon ({'0-5' if unlocked else '0-4'}): {C_RESET}"
        ch = input(prompt).strip()
        if ch == '1':
            if menu_nhap_key(hwid):
                verified = True
            input(f"\n{C_YELLOW}[ENTER] de tiep tuc...{C_RESET}")
        elif ch == '2':
            menu_tao_link(hwid)
            input(f"\n{C_YELLOW}[ENTER] de tiep tuc...{C_RESET}")
        elif ch == '3':
            ...
            if menu_check_duyet(hwid):
                verified = True
            input(f"\n{C_YELLOW}[ENTER] de tiep tuc...{C_RESET}")
        elif ch == '5' and unlocked:
            return True
        elif ch == '0':
            clear_screen()
            sys.exit(0)
        else:
            print(f"{C_RED}[!] Lua chon khong hop le!{C_RESET}")
            time.sleep(0.8)


# ════════════════════════════════════════════════════════════
#  OTP FUNCTIONS GOC TU SP.PY — NGUYEN VEN 100% KHONG CHAM
# ════════════════════════════════════════════════════════════
...
OTP_FUNCTIONS = [
    ...
    send_otp_via_TINIWORLD, send_otp_via_BACHHOAXANH, send_otp_via_mafccomvn, send_otp_via_phuclong
]

def run_spam_tool(hwid):
    """
    Man hinh spam OTP.
    Bam 0 = quay lai menu auth.
    Tra ve True = quay lai, False = het han (can xac thuc lai).
    """
    while True:
        # Kiem tra han truoc moi phien spam
        valid, key, expiry, is_perm = is_auth_valid()
        if not valid:
            notify_expired()
            return False   # quay lai menu xac thuc

        clear_screen()
        print(f"{C_PURPLE}{'='*54}{C_RESET}")
        print(f"{C_CYAN}{C_BOLD}{'  SPAM OTP - DA XAC THUC  ':^54}{C_RESET}")
        print(f"{C_PURPLE}{'='*54}{C_RESET}")
        if key and key != "__da_duyet__":
            print(f"  {C_CYAN}KEY : {C_YELLOW}{key}{C_RESET}")
        if is_perm:
            print(f"  {C_CYAN}HAN : {C_GREEN}Vinh vien{C_RESET}")
        elif expiry and expiry > 0:
            ...
            if left > 0:
                ...
                print(f"  {C_CYAN}CON : {C_RED}HET HAN!{C_RESET}")
        print(f"{C_PURPLE}{'-'*54}{C_RESET}")
        print(f"  {C_RED}0.{C_RESET} Quay lai menu chinh")
        print(f"{C_PURPLE}{'-'*54}{C_RESET}")
        phone = input(f"{C_YELLOW}[?] So dien thoai can spam (0 = menu): {C_RESET}").strip()
        if phone == '0':
            return True   # quay lai menu auth
        if not phone:
            print(f"{C_RED}[!] Chua nhap so dien thoai!{C_RESET}")
            time.sleep(1)
            continue
        try:
            repeat_count = int(input(f"{C_YELLOW}[?] So lan spam: {C_RESET}").strip())
        except ValueError:
            print(f"{C_RED}[!] So lan khong hop le!{C_RESET}")
            time.sleep(1)
            continue
        print(f"\n{C_GREEN}[OK] Bat dau spam {C_YELLOW}{phone}{C_GREEN} x{repeat_count} lan...{C_RESET}\n")
        for i in range(repeat_count):
            # Kiem tra het han moi vong
            vv, _, ex2, p2 = is_auth_valid()
            if not vv:
                print(f"\n{C_RED}[!] HET HAN GIUA CHUNG — DUNG LAI!{C_RESET}")
                clear_auth()
                time.sleep(2)
                return False
            print(f"{C_CYAN}[Vong {i+1}/{repeat_count}]{C_RESET}")
            threads = []
            for func in OTP_FUNCTIONS:
                t = threading.Thread(target=send_otp_with_delay, args=(func, phone, 5))
                threads.append(t)
            for t in threads: t.start()
            for t in threads: t.join()
        print(f"\n{C_GREEN}[OK] Hoan thanh {repeat_count} vong spam!{C_RESET}")
        input(f"{C_YELLOW}[ENTER] tiep tuc | nhap 0 = menu...{C_RESET}")
...
if __name__ == '__main__':
    os.system('cls' if os.name == 'nt' else 'clear')
    HWID = get_hwid()   # Co dinh theo thiet bi, lay tu tool_data.txt
    while True:
        ok = run_auth(HWID)
        if ok:
            back = run_spam_tool(HWID)
            if not back:
                # Het han → menu auth
                continue
            # back=True → nguoi dung bam 0 → quay lai menu auth
            continue
        break
